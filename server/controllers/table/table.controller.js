...
const list = async (req, res) => {
  try {
    ...
    if (current * pagesize < tables.length) {
      tables = tables.slice(
        current * pagesize,
        Math.min((current + 1) * pagesize, tables.length)
      );
    } else {
      tables = [];
    }
...
const update = async (req, res) => {
  try {
    ...
    const tablePoinExists = await Table.findOne({
      tablePoin: req.table.tablePoin,
    });
...
const insertOrderToTable = async (req, res) => {
  try {
    const tableFound = await Table.findOne({ tablePoin: req.table.tablePoin });
    const orderId = req.body.orderId;
    if (tableFound && orderId) {
      tableFound.ordersId = [...tableFound.ordersId, orderId];
      await tableFound.save();
      return res.status(200).json(tableFound);
    } else {
      return res.status(400).json({ error: " Table or Order not found" });
    }
  } catch (err) {
    console.error(err);
    return res.status(400).json({ error: "bad request" });
